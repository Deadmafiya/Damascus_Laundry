//! CI guard: dl-state's value paths must be **float-free**.
//!
//! Same rationale as `dl-feed/tests/fixed_point_no_floats.rs`: any
//! `f32` / `f64` in pool state, decoders, or registry code would break
//! AC-1 (determinism). The display layer in `dl-core` is the only
//! place floats are allowed; this crate is on the value-path side of
//! that boundary.
//!
//! Run with `cargo test -p dl-state --test fixed_point_no_floats`.

use std::path::Path;

#[test]
fn no_floats_in_dl_state_value_path() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    let mut bad = Vec::new();
    visit(&root, &mut bad);
    assert!(bad.is_empty(), "f32/f64 found in dl-state src: {bad:#?}");
}
